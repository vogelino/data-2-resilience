		...
		{#if links.length > 0}
			<div class="mt-6 flex items-center gap-4">
				{#each links as link (link.href)}
					<a
						href={link.href}
						class="inline-flex items-center rounded-md bg-primary px-4 py-2 font-medium text-primary-foreground shadow-sm transition-colors hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
					>
						{link.text}
					</a>
				{/each}
