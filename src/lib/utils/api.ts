...
export const api = (customFetch = fetch) => ({
	getStationsMetadata: async () => {
		const response = await customFetch(
			`${PUBLIC_API_BASE_URL}/v1/stations/metadata`,
		);
	...
	getStationData: async (params: {
		...
		const normalizedStartDate = normalizeDateByScale(
			params.start_date,
			params.scale,
		);
		const normalizedEndDate = normalizeDateByScale(
			params.end_date,
			params.scale,
		);
		...
		const response = await customFetch(
			`${PUBLIC_API_BASE_URL}/v1/data/${params.id}?${urlParams}`,
		);
	...
	getStationsSnapshot: async (params: {
		...
		const response = await customFetch(
			`${PUBLIC_API_BASE_URL}/v1/network-snapshot?${urlParams}`,
		);
...
function normalizeDateByScale(
	date: Date,
	scale: "hourly" | "daily" | "max" | undefined,
) {
	switch (scale) {
		case "hourly":
